...
namespace hyperarrow {
  ...
  static const std::map<int, std::map<const std::string, std::shared_ptr<arrow::Int64Array>>> mapDateArraysToComponents(const std::shared_ptr<arrow::Table> table) {
    std::map<int, std::map<const std::string, std::shared_ptr<arrow::Int64Array>>> result;
    const std::shared_ptr<arrow::Schema> schema = table->schema();
    for (int i = 0; i < schema->num_fields(); i++ ) {
      if (schema->field(i)->type() == arrow::date32()) {
	auto array = std::static_pointer_cast<arrow::Date32Array>(table->column(i)->chunk(0));

	// ARROW_ASSIGN_OR_RAISE(arrow::Datum year_datum, arrow::compute::Year(array));
	// ARROW_ASSIGN_OR_RAISE macro wasn't working. Yielded error:
	// error: could not convert '(& _error_or_value7)->arrow::Result<arrow::Datum>
	// ::status()' from 'const arrow::Status' to 'const std::map<int, std::map<
	// const std::__cxx11::basic_string<char>, std::shared_ptr<arrow::NumericArray<
	// arrow::Int64Type> > > >
	//
	...
	std::shared_ptr<arrow::Int64Array> months = std::dynamic_pointer_cast<arrow::Int64Array>(months_arr);
	std::shared_ptr<arrow::Int64Array> days = std::dynamic_pointer_cast<arrow::Int64Array>(days_arr);

	std::map<const std::string, std::shared_ptr<arrow::Int64Array>> innerResult;
